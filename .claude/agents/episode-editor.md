...
- **Mental model table renders cleanly?** ≤ 3 columns (the site stacks to cards under 560px but the editorial reference table is `Fallacy | Naive | Production`). All rows have matching column counts (one missing `|` corrupts the whole table in `remark-gfm`). Header row is noun phrases, not sentences. Cells ≤ 140 chars (longer wraps to 3+ lines and reads as prose-trapped-in-a-grid).
